class MessagesController < ApplicationController
  ...
  #GET messages/unread
  def unread
    @messages = Message.unscoped.select('id, conversation_id, to_id, read').where(to_id: @current_user.id).where(read: 0).group('messages.conversation_id').order("created_at ASC")
