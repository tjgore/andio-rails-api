class ConversationsController < ApplicationController
  ...
  def index
    @conversations = Conversation.all.except(:id).where("to_id = :current_user_id OR from_id = :current_user_id", {current_user_id: @current_user.id}).joins(:messages).group('conversation_id').take(10).as_json
